...
import processing.opengl.PJOGL;
  ...
  private class MyFluidData implements DwFluid2D.FluidData{
    ...
    public void update(DwFluid2D fluid) {
      ...
      radius = 100;
      temperature = -0.5f;
      fluid.addDensity(px, py, radius, 1.0f, 0.04f, 0.0f, 1.0f); // Dorado
      fluid.addTemperature(px, py, radius, temperature);
      particles.spawn(fluid, px, py, radius, 2000);
      ...
      if(mouse_input && mouseButton == LEFT){
        ...
        particles.spawn(fluid, px, py, radius*2, 1300);
      }
  ...
  //      fluid.param.vorticity               = 0.10f;
   ...
   // pg_obstacles.noFill();
    ...
    particles = new MyParticleSystem(context, 8000 * 8000);
